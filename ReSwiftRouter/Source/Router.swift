...
open class Router<State: StateType>: StoreSubscriber
{
	...
	{
		/* No need to process the state change if the route stays the same! */
		if (_lastNavigationState.route == state.route) { return }
	...
	static func routingActionsForTransitionFrom(_ oldRoute: Route, newRoute: Route) -> [RoutingActions]
	{
		...
		print("[DEBUG] [Routing] Router: [\(oldRoute.pathString)] --\(type)--> [\(newRoute.pathString)]")
