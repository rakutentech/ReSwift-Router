	...
	override func spec()
	{
		describe("routing calls")
		{
			...
			it("calculates transitions from an empty route to a multi segment route")
			{
				...
				let routingActions = Router<AppState>.routingActionsForTransitionFrom(oldRoute, newRoute: newRoute)
				...
				if case let RoutingActions.Push(responsibleRoutableIndex, segmentToBePushed)
				   = routingActions[0]
				{
					if responsibleRoutableIndex == 0 && segmentToBePushed.id == tabBarViewControllerIdentifier
					{
						action1Correct = true
					}
				}
				
				if case let RoutingActions.Push(responsibleRoutableIndex, segmentToBePushed) = routingActions[1]
				{
					if responsibleRoutableIndex == 1 && segmentToBePushed.id == statsViewControllerIdentifier
					{
						action2Correct = true
					}
				}
			...
			{
				let oldRoute = Route([tabBarViewControllerIdentifier, counterViewControllerIdentifier])
				let newRoute = Route([tabBarViewControllerIdentifier, statsViewControllerIdentifier, infoViewControllerIdentifier])
				let routingActions = Router<AppState>.routingActionsForTransitionFrom(oldRoute, newRoute: newRoute)
				var action1Correct:Bool?
				var action2Correct:Bool?
				
				if case let RoutingActions.Change(responsibleRoutableIndex, segmentToBeReplaced, newSegment) = routingActions[0]
				{
					if responsibleRoutableIndex == 1 && segmentToBeReplaced.id == counterViewControllerIdentifier && newSegment.id == statsViewControllerIdentifier
					{
						action1Correct = true
					}
				}
				...
				if case let RoutingActions.Push(responsibleRoutableIndex, segmentToBePushed) = routingActions[1]
				{
					if responsibleRoutableIndex == 2 && segmentToBePushed.id == infoViewControllerIdentifier
					{
						action2Correct = true
					}
				}
			...
			{
				let oldRoute = Route([tabBarViewControllerIdentifier, statsViewControllerIdentifier, counterViewControllerIdentifier])
				let newRoute = Route([tabBarViewControllerIdentifier])
				let routingActions = Router<AppState>.routingActionsForTransitionFrom(oldRoute, newRoute: newRoute)
				var action1Correct:Bool?
				var action2Correct:Bool?
				
				if case let RoutingActions.Pop(responsibleRoutableIndex, segmentToBePopped) = routingActions[0]
				{
					if responsibleRoutableIndex == 2 && segmentToBePopped.id == counterViewControllerIdentifier
					{
						action1Correct = true
					}
				}
				
				if case let RoutingActions.Pop(responsibleRoutableIndex, segmentToBePopped) = routingActions[1]
				{
					if responsibleRoutableIndex == 1 && segmentToBePopped.id == statsViewControllerIdentifier
					{
						action2Correct = true
					}
				}
			...
			{
				let oldRoute = Route([tabBarViewControllerIdentifier])
				let newRoute = Route([tabBarViewControllerIdentifier, statsViewControllerIdentifier, counterViewControllerIdentifier])
				let routingActions = Router<AppState>.routingActionsForTransitionFrom(oldRoute, newRoute: newRoute)
				var action1Correct:Bool?
				var action2Correct:Bool?
				
				if case let RoutingActions.Push(responsibleRoutableIndex, segmentToBePushed) = routingActions[0]
				{
					if responsibleRoutableIndex == 1 && segmentToBePushed.id == statsViewControllerIdentifier
					{
						action1Correct = true
				...
				if case let RoutingActions.Push(responsibleRoutableIndex, segmentToBePushed) = routingActions[1]
				{
					if responsibleRoutableIndex == 2 && segmentToBePushed.id == counterViewControllerIdentifier
					{
						action2Correct = true
					}
				}
